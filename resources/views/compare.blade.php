@extends('layout')
@section('title')
    <title>{{ __('translate.Compare') }}</title>
@endsection
...
<main>
     ...
     <div class="inner-banner-img" style=" background-image: url({{ asset($breadcrumb) }}) ;"></div>
        ...
        <div class="col-lg-12">
            <div class="inner-banner-df">
                <h1 class="inner-banner-taitel">{{ __('translate.Compare') }}</h1>
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="{{ route('home') }}">{{ __('translate.Home') }}</a></li>
                        <li class="breadcrumb-item active" aria-current="page">{{ __('translate.Compare') }}</li>
                    </ol>
                </nav>
            </div>
            </div>
    ...
    @if ($compare_qty > 0)
        <section class="car-compare">
            <div class="container">
                <div class="row g-5 ">
                    @php
                        $lg_size = 12 / $compare_qty;
                        if($compare_qty == 1){
                            $sm_size = 12;
                        }elseif ($compare_qty > 1) {
                            $sm_size = 6;
                        }

                        $percentage = 100 / ($compare_qty + 1);

                    @endphp

                    @foreach ($cars as $car)
                        <div class="col-lg-{{ $lg_size }} col-sm-{{ $sm_size }} col-md-{{ $sm_size }}">
                            <div class="car-compare-item">
                                <div class="car-compare-img">
                                    <img src="{{ asset($car->thumb_image) }}" alt="img">

                                    <button type="button" class="delet" onclick="removeCompare({{ $car->id }})">
                                        <i class="fa-solid fa-x"></i>
                                    </button>
                                </div>

                                <form action="{{ route('remove-to-compare', $car->id) }}" id="remove_compare_{{ $car->id }}" class="d-none" method="POST">
                                    @csrf
                                    @method('DELETE')

                                </form>

                                <div class="car-compare-item-text">
                                    <p>
                                        @if ($car->offer_price)
                                    ...
                                    <h3><a href="{{ route('listing', $car->slug) }}">{{ html_decode($car->title) }} </a></h3>
                                </div>
                                <a href="{{ route('listing', $car->slug) }}" class="thm-btn-two"> {{ __('translate.View Details') }}</a>
                            </div>
                        </div>
                    @endforeach
                </div>
                ...
                <div class="row">
                    <div class="col-lg-12">
                        ...
                        <div class="accordion two " id="accordionPanelsStayOpenExample2" data-aos="fade-up"
                            data-aos-delay="50">
                                ...
                                <div id="panelsStayOpen-collapsetwo" class="accordion-collapse collapse show"
                                    ...
                                    <div class="accordion-body">
                                        <table class="table table-bordered ">
                                            ...
                                            <tr>
                                                <td>{{ __('translate.Rent Period') }}</td>

                                                @foreach ($cars as $car)
                                                    <td>
                                                        @if ($car->rent_period == 'Hourly')
                                                            {{ __('translate.Hourly') }}
                                                        @elseif ($car->rent_period == 'Daily')
                                                            {{ __('translate.Daily') }}
                                                        @elseif ($car->rent_period == 'Monthly')
                                                            {{ __('translate.Monthly') }}
                                                        @elseif ($car->rent_period == 'Yearly')
                                                            {{ __('translate.Yearly') }}
                                                        @else
                                                            ---
                                                        @endif
                                            ...
                                            <tr>
                                                <td>{{ __('translate.Interior Color') }}</td>
                                                @foreach ($cars as $car)
                                                    <td>
                ...
                <div class="row justify-content-center">
                    <div class="col-lg-7">
                        <div class="not-found-box">
                            ...
                            <h4 class="not-found-txt">{{ __('translate.Compare Item Not Found!') }}</h4>
                            <p class="not-found-sub-txt"> {{ __('translate.Your compare list is empty, to add compare item click the button below') }} </p>
